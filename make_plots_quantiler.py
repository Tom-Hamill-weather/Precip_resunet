"""
python make_plots_quantiler.py cyyyymmddhh clead
e.g.,
python make_plots_quantiler.py 2025120812 12

After inference is run with resunet_inference_quantiler.py, this loads
the netCDF files produced by quantile regression inference and makes plots.

This version reads the Quantile model probability files with the "_quantiler" suffix.
"""
...
def probability_read(clead, cyyyymmddhh, GRAFprobsdir_conus_laptop):
    """Read quantile model probability files."""
    ...
    if fexist == True:
        ...
        # Read quantile model probabilities
        quantile_p0p25mm_prob = nc.variables['quantile_p0p25mm_prob'][:,:]
        quantile_p1mm_prob = nc.variables['quantile_p1mm_prob'][:,:]
        quantile_p2p5mm_prob = nc.variables['quantile_p2p5mm_prob'][:,:]
        quantile_p5mm_prob = nc.variables['quantile_p5mm_prob'][:,:]
        quantile_p10mm_prob = nc.variables['quantile_p10mm_prob'][:,:]
        ...
        quantile_p0p25mm_prob = np.empty((0,0), dtype=float)
        raw_p1mm_prob = np.empty((0,0), dtype=float)
        quantile_p1mm_prob = np.empty((0,0), dtype=float)
        raw_p2p5mm_prob = np.empty((0,0), dtype=float)
        quantile_p2p5mm_prob = np.empty((0,0), dtype=float)
        raw_p5mm_prob = np.empty((0,0), dtype=float)
        quantile_p5mm_prob = np.empty((0,0), dtype=float)
        raw_p10mm_prob = np.empty((0,0), dtype=float)
        quantile_p10mm_prob = np.empty((0,0), dtype=float)

    return istat_prob, raw_p0p25mm_prob, quantile_p0p25mm_prob, raw_p1mm_prob, \
        quantile_p1mm_prob, raw_p2p5mm_prob, quantile_p2p5mm_prob, raw_p5mm_prob, \
        quantile_p5mm_prob, raw_p10mm_prob, quantile_p10mm_prob, lat, lon

# -------------------------------------------------------------------------

def plot_figure(lat_1, lat_2, lat_0, lon_0, lons, lats,
        cyyyymmddhh, clead, precip_GRAF, lowprob_raw, lowprob_quantile,
        highprob_raw, highprob_quantile, ltitle, htitle, GRAF_plot_dir):
    """Make a 4-panel plot with GRAF forecast and probabilities."""
    ...
    CS2 = m.contourf(x, y, precip_GRAF, clevs, \
    ...
    # --- panel 2: Quantile low prob
    axloc = [0.51,0.58,0.48,0.34]
    ax1 = fig.add_axes(axloc)
    title = '(b) Quantile Regression '+ltitle
    ax1.set_title(title, fontsize=12,color='Black')
    CS2 = m.contourf(x, y, lowprob_quantile, clevs_prob, \
        cmap=None, colors=colorst, extend='both')
    m.drawcoastlines(linewidth=0.6,color='Gray')
    m.drawcountries(linewidth=0.4,color='Gray')
    m.drawstates(linewidth=0.2,color='Gray')
    ...
    # --- panel 3: Quantile high prob
    axloc = [0.01,0.12,0.48,0.34]
    ax1 = fig.add_axes(axloc)
    title = '(c) Quantile Regression '+htitle
    ax1.set_title(title, fontsize=12,color='Black')
    CS2 = m.contourf(x, y, highprob_quantile, clevs_prob, \
        cmap=None, colors=colorst, extend='both')
    m.drawcoastlines(linewidth=0.6,color='Gray')
    m.drawcountries(linewidth=0.4,color='Gray')
    m.drawstates(linewidth=0.2,color='Gray')
    ...
    plot_title = GRAF_plot_dir + 'ResUnet_GRAF_probs_IC' + \
         cyyyymmddhh+'_lead'+clead+'h_quantiler.png'
...
def plot_small_figure(lat_1, lat_2, lat_0, lon_0, lons, lats,
        llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat,
        cyyyymmddhh, clead, precip_GRAF, lowprob_raw, lowprob_quantile,
        highprob_raw, highprob_quantile, ltitle, htitle, GRAF_plot_dir):
    """Make a smaller 4-panel plot zoomed to region of interest."""
...
if len(sys.argv) < 3:
    print("Usage: python make_plots_quantiler.py <YYYYMMDDHH> <lead>")
    sys.exit(1)
...
istat_prob, raw_p0p25mm_prob, quantile_p0p25mm_prob, raw_p1mm_prob, \
    quantile_p1mm_prob, raw_p2p5mm_prob, quantile_p2p5mm_prob, raw_p5mm_prob, \
    quantile_p5mm_prob, raw_p10mm_prob, quantile_p10mm_prob, lat, lon = \
...
if istat_GRAF == 0 and istat_prob == 0:
    ...
    print(f'P(X > 0.25mm): Raw mean={np.mean(raw_p0p25mm_prob):.3f}, '
          f'Quantile mean={np.mean(quantile_p0p25mm_prob):.3f}')
    print(f'P(X > 1mm):    Raw mean={np.mean(raw_p1mm_prob):.3f}, '
          f'Quantile mean={np.mean(quantile_p1mm_prob):.3f}')
    print(f'P(X > 5mm):    Raw mean={np.mean(raw_p5mm_prob):.3f}, '
          f'Quantile mean={np.mean(quantile_p5mm_prob):.3f}')
    print(f'P(X > 10mm):   Raw mean={np.mean(raw_p10mm_prob):.3f}, '
          f'Quantile mean={np.mean(quantile_p10mm_prob):.3f}')
    ...
    if cyyyymmddhh == '2025120412':
        ...
        lowprob_quantile = quantile_p0p25mm_prob
        highprob_quantile = quantile_p2p5mm_prob
        ...
        lowprob_quantile = quantile_p2p5mm_prob
        highprob_quantile = quantile_p5mm_prob
    ...
    istat = plot_figure(lat_1, lat_2, lat_0, lon_0, lons, lats, \
        cyyyymmddhh, clead, precipitation_GRAF, lowprob_raw, lowprob_quantile, \
        highprob_raw, highprob_quantile, ltitle, htitle, GRAF_plot_dir)

    istat = plot_small_figure(lat_1, lat_2, lat_0, lon_0, lons, lats, \
        llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat, \
        cyyyymmddhh, clead, precipitation_GRAF, lowprob_raw, lowprob_quantile, \
        highprob_raw, highprob_quantile, ltitle, htitle, GRAF_plot_dir)
