"""
python resunet_inference_gamma_mixture_noterrain_fulldomain.py cyyyymmddhh clead
e.g.,
python resunet_inference_gamma_mixture_noterrain_fulldomain.py 2025120412 12

*** NO-TERRAIN ABLATION VARIANT ***
Fork of resunet_inference_gamma_mixture_fulldomain.py that runs the no-terrain
model (3 input channels: GRAF precip, GFS RH, GRAF x RH). It loads the v1
architecture from pytorch_train_resunet_gamma_mixture_noterrain, selects
weights matching the prefix "resunet_gamma_mixture_noterrain_", and writes
output to "<date>_<lead>_probs_gamma_mixture_noterrain.nc" so it never collides
with the full 7-channel probabilities.
...
Input features (3 channels) -- NO TERRAIN:
- GRAF precipitation
...
from pytorch_train_resunet_gamma_mixture_noterrain import AttnResUNet
...
def generate_features(nchannels, date, clead,
    ...
    """
    Generate 3-channel feature array for model input and return as GPU tensor.

    NO-TERRAIN channels: GRAF precip, GFS RH, GRAF x RH.
    The terrain args (t_diff, dt_dlon, dt_dlat) are accepted for call
    ...
    channels = np.stack([
        ...
        normalize_stats(gfs_rh,                          1),
        normalize_stats(precipitation_GRAF * gfs_rh,     2),
...
def read_pytorch(cyyyymmddhh, clead):
    ...
    glob_pattern = os.path.join(TRAIN_DIR, "resunet_gamma_mixture_noterrain_*_best.pth")
    files = glob.glob(glob_pattern)

    if not files:
        print(f"   No no-terrain Gamma mixture training files in {TRAIN_DIR} match pattern")
    ...
    for fpath in files:
        ...
        match = re.search(
            r"resunet_gamma_mixture_noterrain_(\d{10})_(\d+)h_best\.pth", basename)
        if match:
    ...
    model = AttnResUNet(in_channels=3, num_outputs=6)
    try:
...
def calc_gamma_probabilities_fulldomain(model, Xpredict_tensor, ny, nx,
    ...
    If save_attention=True, the finest-scale attention map (att4, same spatial
    resolution as the output) is returned as an extra numpy array.
    """
    ...
    with torch.no_grad():
        if save_attention:
            raw_logits, attention_maps = model(Xpad, return_attention=True)
        else:
            raw_logits = model(Xpad)
            attention_maps = None
    ...
    # att4 is the finest-scale gate (Up4), same spatial resolution as the output.
    # att1-att3 are at 1/8, 1/4, 1/2 resolution respectively.
    attn4_np = None
    if attention_maps is not None:
        attn4 = attention_maps[3][0, 0, :ny, :nx]  # squeeze batch and channel dims
        attn4_np = attn4.float().cpu().numpy()
        attn4_np[:clip, :] = np.nan;  attn4_np[-clip:, :] = np.nan
        attn4_np[:, :clip] = np.nan;  attn4_np[:, -clip:] = np.nan

    return (gamma_probs_np, p0_np, w_np, a1_np, t1_np, a2_np, t2_np, attn4_np)
...
def write_probabilities_to_netcdf(filename, lats, lons, raw_probs, gamma_probs,
    ...
    try:
        ...
        if attention_map is not None:
            attn_var = ncfile.createVariable('attn_gate4', 'f4', ('y', 'x'),
                                             zlib=True, complevel=4,
                                             least_significant_digit=4,
                                             fill_value=np.float32(9.96921e+36))
            attn_var[:] = np.ma.masked_invalid(attention_map)
            attn_var.long_name = ('Attention gate weight nu at finest decoder scale (Up4); '
                                  'values near 1 indicate regions the model attends to, '
                                  'values near 0 indicate suppressed regions')
            attn_var.units = '1 (dimensionless, 0-1 range)'
            attn_var.note = ('Spatial average of the single-channel sigmoid gate output from '
                             'the Up4 AttentionGate, which operates on 96x96-scale encoder '
                             'features.  att1 (coarsest, 1/8 resolution) through att3 (1/2) '
                             'are not stored; re-run with return_attention=True to access all.')

        ncfile.description = \
            "Precipitation probabilities (Raw GRAF vs 2-Component Gamma Mixture) - FULL DOMAIN"
        ncfile.history = "Generated by resunet_inference_gamma_mixture_noterrain_fulldomain.py"
        ncfile.close()
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python resunet_inference_gamma_mixture_noterrain_fulldomain.py <YYYYMMDDHH> <lead>")
        sys.exit(1)
    ...
    ny = 1308; nx = 1524
    nchannels = 3   # NO TERRAIN: GRAF, GFS RH, GRAF x RH
