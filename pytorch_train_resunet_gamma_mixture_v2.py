"""
pytorch_train_resunet_gamma_mixture_v2.py

Usage example:

$ python pytorch_train_resunet_gamma_mixture_v2.py 2025120100 12

where you supply the YYYYMMDDHH of initial condition and lead time in h.

This is v2 of the gamma mixture training script. Changes from v1
(pytorch_train_resunet_gamma_mixture.py):

  1. Bilinear + Conv2d upsampling replaces ConvTranspose2d in all four decoder
     stages. ConvTranspose2d with kernel_size==stride produces uneven overlap
     that causes a checkerboard pattern in inferred probability fields. Bilinear
     interpolation followed by a standard 3x3 convolution avoids this entirely.

  2. BottleneckResidualBlock (1x1 -> 3x3 -> 1x1) replaces ResidualBlock
     (3x3 -> 3x3). The 1x1 layers compress and restore channel depth around
     the spatial 3x3, cutting parameters and FLOPs substantially at the higher
     channel counts in the bridge and early decoder stages.

Everything else — loss function, dataset, training loop, AMP, checkpointing,
normalization, initialization — is unchanged from v1.

Checkpoint files use the prefix "resunet_gamma_mixture_v2_" so they are never
accidentally loaded in place of v1 weights (the architectures are incompatible).
...
(3) shape1 (a1): First Gamma shape parameter (a1 > 0) - light precipitation
(4) scale1 (t1): First Gamma scale parameter (t1 > 0) - light precipitation
(5) shape2 (a2): Second Gamma shape parameter (a2 > a1) - heavy precipitation
(6) scale2 (t2): Second Gamma scale parameter (t2 > 0) - heavy precipitation

For positive precipitation amounts, the distribution is a mixture of two Gammas:
    PDF(x) = w * Gamma(x; a1, t1) + (1-w) * Gamma(x; a2, t2)

Complete model:
    P(X = 0) = p0
    P(X = x > 0) = (1 - p0) * [w * Gamma(x; a1, t1) + (1-w) * Gamma(x; a2, t2)]
...
enforce a HARD ORDERING CONSTRAINT:
    ...
    a2 = a1 + softplus(shape_offset) + 0.5
...
**Case 2: Observation y > 0**
    NLL = -log(1 - p0) + NLL_mixture

    where:
    NLL_mixture = -log[w * pdf_gamma(y; a1, t1) + (1-w) * pdf_gamma(y; a2, t2)]

    pdf_gamma(y; a, t) = exp(-NLL_gamma(y; a, t))
    NLL_gamma(y; a, t) = lgamma(a) - (a-1)*log(y) + (a-1)*log(t) + y/t + log(t)
...
2. Fit 2-component Gamma mixture to wet pixels using EM:
   - Returns: weights (w, 1-w), shapes (a1, a2), scales (t1, t2)
3. Initialize biases:
   - bias[1] = logit(w)
   - bias[2] = inverse_softplus(a1 - shape_min)
   - bias[3] = inverse_softplus(t1 - scale_min)
   - bias[4] = inverse_softplus(a2 - a1 - 0.5)  # offset for hard constraint
   - bias[5] = inverse_softplus(t2 - scale_min)
...
Attention Residual U-Net with bottleneck blocks and bilinear upsampling.
Output: 6 channels for 2-component Gamma mixture parameters.
...
(4) Interaction: GRAF x terrain elevation deviation
(5) Interaction: GRAF x GFS relative humidity
...
v2 architectural changes: June 2026
"""
...
if DEVICE.type == 'cpu':
    ...
    BATCH_SIZE = 128  # L4 has 23 GB; batch=128 direct is faster than 16x8 accumulation
    NUM_WORKERS = 6
    USE_AMP = True
...
class BottleneckResidualBlock(nn.Module):
    ...
    The first 1x1 conv compresses channels by `reduction`, the 3x3 conv
    processes spatial features at reduced channel depth, and the final 1x1
    restores the original channel count. This cuts FLOPs and parameters
    substantially at high channel counts while preserving representational
    capacity.

    Replaces the standard 3x3->3x3 ResidualBlock used in v1.
    """
...
class AttentionGate(nn.Module):
    ...
    def forward(self, g, x):
        ...
        psi = self.relu(g1 + x1)
        psi = self.psi(psi)
        self._alpha = psi  # spatial attention map in (0,1); shape [B,1,H,W]
        return x * psi


def _bilinear_upsample(in_channels, out_channels):
    """
    2x bilinear upsample followed by a 3x3 conv.

    Replaces ConvTranspose2d(in, out, kernel_size=2, stride=2) used in v1.
    ConvTranspose2d produces uneven pixel overlap when kernel_size == stride,
    leading to a checkerboard pattern in the output. Bilinear interpolation
    has no such overlap bias; the following Conv2d learns the refinement.
    """
...
class AttnResUNet(nn.Module):
    """
    Attention Residual U-Net — v2.

    Differences from v1:
    - BottleneckResidualBlock instead of ResidualBlock in all conv stages
    - Bilinear+Conv2d upsampling instead of ConvTranspose2d in all decoder stages
    ...
    def __init__(self, in_channels=7, num_outputs=6):
        ...
        # Encoder
        self.inc   = BottleneckResidualBlock(in_channels, 64)
        self.down1 = nn.Sequential(nn.MaxPool2d(2), BottleneckResidualBlock(64, 128))
        self.down2 = nn.Sequential(nn.MaxPool2d(2), BottleneckResidualBlock(128, 256))
        self.down3 = nn.Sequential(nn.MaxPool2d(2), BottleneckResidualBlock(256, 512))

        # Bridge
        self.down4 = nn.Sequential(nn.MaxPool2d(2), BottleneckResidualBlock(512, 1024))

        # Decoder — bilinear upsample + Conv2d, then attention + bottleneck block
        self.up1     = _bilinear_upsample(1024, 512)
        self.att1    = AttentionGate(F_g=512, F_l=512, F_int=256)
        self.upconv1 = BottleneckResidualBlock(1024, 512)

        self.up2     = _bilinear_upsample(512, 256)
        self.att2    = AttentionGate(F_g=256, F_l=256, F_int=128)
        self.upconv2 = BottleneckResidualBlock(512, 256)

        self.up3     = _bilinear_upsample(256, 128)
        self.att3    = AttentionGate(F_g=128, F_l=128, F_int=64)
        self.upconv3 = BottleneckResidualBlock(256, 128)

        self.up4     = _bilinear_upsample(128, 64)
        self.att4    = AttentionGate(F_g=64, F_l=64, F_int=32)
        self.upconv4 = BottleneckResidualBlock(128, 64)

        # Final output layer — 6 channels for 2-component mixture model
...
class GammaMixtureNLLLoss(nn.Module):
    ...
    - With probability p0: zero precipitation
    - With probability (1-p0): Mixture of two Gammas
        - With weight w: Gamma(a1, t1) - light precipitation
        - With weight (1-w): Gamma(a2, t2) - heavy precipitation
    ...
    def forward(self, logits, targets):
        ...
        shape1 = self.shape_min + F.softplus(logits[:, 2, :, :])  # [shape_min, inf)
        scale1 = self.scale_min + F.softplus(logits[:, 3, :, :])  # [scale_min, inf)
        ...
        scale2 = self.scale_min + F.softplus(logits[:, 5, :, :])  # [scale_min, inf)
        ...
        if is_positive.any():
            ...
            # NLL for zero-inflation: -log(1 - p0)
            ...
            # log(w * pdf1 + (1-w) * pdf2) = logsumexp([log(w) + log_pdf1, log(1-w) + log_pdf2])
...
class GRAF_Dataset(Dataset):
    ...
    Same as v1. Supports both pickle (.cPick) and NetCDF (.nc) formats.
    """
    def __init__(self, pickle_file, normalization_stats=None, train=False, power_transform=1.0):
        ...
        # Compute GRAF x RH interaction from raw/transformed values before normalization
        ...
        if normalization_stats is None:
            ...
            if self.power_transform == 1.0:
                maxs[3] = max(maxs[3], 35000.0)  # GRAF x terrain
                maxs[4] = max(maxs[4], 7500.0)   # GRAF x RH
            else:
...
def compute_gamma_mixture_climatology(train_dataset):
    ...
    print(f"    Shape (a1): {shape1:.3f}")
    print(f"    Scale (t1): {scale1:.3f}")
    print(f"    Mean: {shape1 * scale1:.3f} mm")
    print(f"  Component 2 (heavy precipitation):")
    print(f"    Weight: {1-weight1:.3f}")
    print(f"    Shape (a2): {shape2:.3f}")
    print(f"    Scale (t2): {scale2:.3f}")
...
def print_diagnostics(epoch, batch_idx, loss_val, logits, targets,
    ...
    if epoch == 0 and batch_idx == 0:
        print("\n" + "="*82)
        print("DIAGNOSTIC OUTPUT EXPLANATION - 2-COMPONENT MIXTURE MODEL (v2)")
        print("="*82)
    ...
    with torch.no_grad():
        ...
        if valid.sum() > 0:
            ...
            print(f"    Shape (a1):  {alpha1:.3f}")
            print(f"    Scale (t1):  {theta1:.3f}")
            print(f"    Mean:        {mean1:.3f} mm")
            print(f"    Std:         {std1:.3f} mm")
            print(f"  Component 2 (heavy) - weight: {1-w:.3f}")
            print(f"    Shape (a2):  {alpha2:.3f}")
            print(f"    Scale (t2):  {theta2:.3f}")
        ...
        def run_synthetic(precip_mm, rh_pct):
            ...
            f2 = rh_pct             # RH
            f3 = p_val * t_val      # GRAF x terrain
            f4 = p_val * rh_pct     # GRAF x RH
            f5 = 0.0                # dlon
            f6 = 0.0                # dlat
        ...
        print(f"  Comp1: a1={syn_0mm['alpha1']:.2f}, t1={syn_0mm['theta1']:.2f}, mean={syn_0mm['mean1']:.2f}mm")
        print(f"  Comp2: a2={syn_0mm['alpha2']:.2f}, t2={syn_0mm['theta2']:.2f}, mean={syn_0mm['mean2']:.2f}mm")
        ...
        print(f"  Comp1: a1={syn_1mm['alpha1']:.2f}, t1={syn_1mm['theta1']:.2f}, mean={syn_1mm['mean1']:.2f}mm")
        print(f"  Comp2: a2={syn_1mm['alpha2']:.2f}, t2={syn_1mm['theta2']:.2f}, mean={syn_1mm['mean2']:.2f}mm")
...
def train_model(date_str, lead_time_str):
    """
    Main training function for 2-component Gamma mixture model (v2 architecture).
    """
    print("\n" + "="*70)
    print(f"Training ResUNet 2-COMPONENT GAMMA MIXTURE MODEL - v2")
    print(f"  Architecture changes vs v1:")
    print(f"    - BottleneckResidualBlock (1x1->3x3->1x1) replaces ResidualBlock")
    print(f"    - Bilinear+Conv2d upsampling replaces ConvTranspose2d")
    ...
    print(f"  Output: 6 parameters (p0, w, a1, t1, a2, t2)")
    ...
    # Create v2 model with 6 outputs
    ...
    # v2 prefix ensures v1 weights are never accidentally loaded
    checkpoint_path = f"{TRAIN_DIR}/resunet_gamma_mixture_v2_{date_str}_{lead_time_str}h_best.pth"
    ...
    if os.path.exists(checkpoint_path):
        print(f"\nFound existing v2 checkpoint: {checkpoint_path}")
        try:
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint = {
                ...
                'architecture': 'v2'  # tag so inference scripts can verify
            }
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python pytorch_train_resunet_gamma_mixture_v2.py YYYYMMDDHH lead_time")
        print("Example: python pytorch_train_resunet_gamma_mixture_v2.py 2025120100 12")
        sys.exit(1)
