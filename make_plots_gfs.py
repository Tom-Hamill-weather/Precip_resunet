"""
python make_plots_gfs.py cyyyymmddhh clead
e.g.,
python make_plots_gfs.py 2025120812 12

After inference is run with resunet_inference_gfs.py, this loads
the netCDF files produced by GFS-enhanced inference and makes plots.
It's a bit hard-coded to specific cases right now.

This version reads the GFS-enhanced probability files with the "_gfs" suffix.
"""
...
def probability_read(clead, cyyyymmddhh, GRAFprobsdir_conus_laptop):
    ...
    # Updated to read GFS-enhanced probability files
...
def plot_GRAF(lat_1, lat_2, lat_0, lon_0, lons, lats, \
    ...
    ax1 = fig.add_axes(axloc)
    title = '(b) Attention ResUNet+GFS '+ltitle
    ax1.set_title(title, fontsize=12,color='Black')
    ...
    ax1 = fig.add_axes(axloc)
    title = '(c) Attention ResUNet+GFS '+htitle
    ax1.set_title(title, fontsize=12,color='Black')
    ...
    # ---- set plot title with _gfs suffix
    plot_title = GRAF_plot_dir + 'ResUnet_GRAF_probs_IC' + \
         cyyyymmddhh+'_lead'+clead+'h_gfs.png'
