"""
python make_plots_gamma_mixture2_fulldomain.py cyyyymmddhh clead
e.g.,
python make_plots_gamma_mixture2_fulldomain.py 2025120812 12

Four-panel plot for full-domain inference output:
...
Reads netCDF files produced by resunet_inference_gamma_mixture_fulldomain.py.
Border pixels are stored as the netCDF fill value (missing data) and are
masked before plotting so they are not drawn.
"""
...
def probability_read(clead, cyyyymmddhh, GRAFprobsdir_conus):
    ...
    # masked_invalid ensures fill-value / NaN border pixels are not plotted
...
def plot_GRAF2(lat_1, lat_2, lat_0, lon_0, lons, lats,
    ...
    plt.suptitle(clead_minus + ' to ' + clead +
        '-h GRAF+GFS-based forecasts, valid ' + datestring, fontsize=14, y=0.975)

    # --- panel (a): GRAF hourly precipitation
    axloc = [0.01, 0.545, 0.48, 0.38]
    ...
    plot_title = GRAF_plot_dir + 'ResUnet_GRAF_terrain_probs_IC' + \
        cyyyymmddhh + '_lead' + clead + 'h_gamma_fulldomain.png'
