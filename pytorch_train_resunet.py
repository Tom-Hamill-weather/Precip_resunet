...
Latest version 15 Dec 2025 (Cleaned & Bug Fixed)

Were more data available, we should try additional predictors, including
(1) PWAT
(2) PWAT * V925 dot GRAD terrain elevation (i.e., upslope of moisture)
(3) CAPE * low level convergence
...
class AttnResUNet(nn.Module):
    def __init__(self, in_channels=5, num_classes=NUM_CLASSES):
        ...
        # Initial dimensions: (Batch, 5, 96, 96)
...
class GRAF_Dataset(Dataset):
    def __init__(self, pickle_file, thresholds=THRESHOLDS, normalization_stats=None, train=False):
        ...
        if normalization_stats is None:
            ...
            maxs[0] = 75.0
            maxs[1] = max(maxs[1], 35000.0)
            maxs[2] = max(maxs[2], 2500.0)
            maxs[3] = max(maxs[3], 0.02)
            maxs[4] = max(maxs[4], 0.02)
    ...
    def __getitem__(self, idx):
        x = np.stack([self.graf[idx], self.terdiff_graf[idx], self.diff[idx], self.dlon[idx], self.dlat[idx]], axis=0)
        y_raw = self.mrms[idx]
...
def train_model(date_str, lead_time_str):
    ...
    train_file = glob.glob(train_pattern)[0]
    val_file = glob.glob(val_pattern)[0]
    ...
    model = AttnResUNet(in_channels=5, num_classes=NUM_CLASSES).to(DEVICE)
    ...
    # Resume Logic
    resume_pattern = f"{TRAIN_DIR}/resunet_ordinal_{date_str}_{lead_time_str}h_epoch_*.pth"
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{NUM_EPOCHS} | Val Loss: {avg_val:.4f} | LR: {optimizer.param_groups[0]['lr']:.2e}")

        # Save Checkpoint
        save_path = f"{TRAIN_DIR}/resunet_ordinal_{date_str}_{lead_time_str}h_epoch_{epoch+1}.pth"
        torch.save({
            'epoch': epoch + 1,
