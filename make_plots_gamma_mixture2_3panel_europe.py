"""
python make_plots_gamma_mixture2_3panel_europe.py cyyyymmddhh clead
e.g.,
python make_plots_gamma_mixture2_3panel_europe.py 2025031112 12

Three-panel horizontal plot for the European domain:
  (a) GRAF hourly precipitation amount
  (b) Attention ResUNet Gamma mixture P(>0.25 mm/h)
  (c) Smoothed raw ensemble P(>0.25 mm/h)

Map aspect ratio for the European LCC domain is ~0.924 (taller than wide),
so the figure is sized narrower than the CONUS equivalent.
"""
...
def read_config_file(config_file, directory_object_name):
    ...
    return GRAFdatadir_europe, GRAFprobsdir, GRAF_plot_dir
...
def plot_3panel_europe(lat_1, lat_2, lat_0, lon_0,
                       lons, lats,
    ...
    # Probability color scale (all levels shown as ticks)
    clevs_prob = [0, 0.02, .1, .2, .3, .4, .5, .6, .7, 0.8, .9, .95, .97, 1.]

    clead_minus = str(int(clead) - 1)

    # ----------------------------------------------------------------
    # Layout  (12" wide x 5" tall)
    # Map aspect ratio for the European LCC domain is ~0.924 (taller than wide).
    # Each panel is 0.300 wide x 0.742 tall → 3.60" x 3.71", aspect 0.970 ≈ 0.924.
    # Three panels + 2 gaps (0.015 each) + margins fill 1.00 horizontally.
    # Panel title fontsize=12 (vs 14 in CONUS) to fit the narrower panels;
    # both are larger than the original 11pt.
    #
    ...
    #   0.862 – 0.975 panel titles + suptitle (~0.57", snug fit for fontsize 20+12)
    # ----------------------------------------------------------------

    pan_left   = [0.030, 0.345, 0.660]   # gap between panels = 0.015 (0.18")
    pan_bot    = 0.12
    pan_w      = 0.300
    pan_h      = 0.742
    cbar_y     = 0.060
    ...
    outfile = (GRAF_plot_dir + 'ResUnet_3panel_Europe_IC' +
...
GRAFdatadir_europe, GRAFprobsdir, GRAF_plot_dir = \
...
if istat_GRAF == 0:
    # Use actual SW/NE corners of the LCC grid (see make_plots_gamma_mixture2_europe.py
    # for why lonmin/latmin cannot be mixed as llcrnr coordinates on this projection).
    llcrnrlon = lons[0,  0]
    llcrnrlat = lats[0,  0]
    urcrnrlon = lons[-1, -1]
    urcrnrlat = lats[-1, -1]

    plot_3panel_europe(lat_1, lat_2, lat_0, lon_0,
                       lons, lats,
                       llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat,
                       cyyyymmddhh, clead, precipitation_GRAF,
                       gamma_p0p25mm_prob, raw_p0p25mm_prob,
