"""
python plot_relia_gamma_year_compare_season.py

Plots the Attention ResUNet (Season+FiLM, gamma) reliability curve for
matched Jan/Apr/Jun 2025 vs. 2026 on the same axes, one PNG per threshold
-- the season-model companion to plot_relia_gamma_year_compare.py (which
covers the baseline per-lead/per-month-retrained model). Reuses the
cPickle files already produced by reliability_resunet_mixture.py with
model_tag='season'; no new inference/reading.
"""
...
FILE_PATTERN = {
    2025: RELIA_DIR + '/relia_GRAF_ResUNet_Mixture_Season_q0.5_2025010100_to_2025063018_lead{}h.cPick',
    2026: RELIA_DIR + '/relia_GRAF_ResUNet_Mixture_Season_q0.5_2026010100_to_2026063018_lead{}h.cPick',
}
...
for clead in LEADS:
    ...
    for ithresh, thresh in enumerate(pthresholds):
        ...
        ctitle = str(clead) + r'-h Attention ResUNet (Season+FiLM) reliability, P(obs $\geq$ ' + ctthresh + ')'
        ...
        outfile = f'Relia_ResUNet_Season_year_compare_2025_vs_2026_{ctthresh}_{clead}h.png'
