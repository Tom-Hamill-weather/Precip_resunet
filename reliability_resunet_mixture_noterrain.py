"""
python reliability_resunet_mixture_noterrain.py clead

e.g.,

python reliability_resunet_mixture_noterrain.py 12

    clead = sys.argv[1]

*** NO-TERRAIN ABLATION VARIANT ***
Fork of reliability_resunet_mixture.py that evaluates the NO-TERRAIN model.
It reads "<date>_<lead>_probs_gamma_mixture_noterrain.nc" and writes its
output cPickle as "relia_GRAF_ResUNet_Mixture_noterrain_q0.5_..." so the
no-terrain BSS does not overwrite the full-model reliability file. The
'BSS_gamma'/'BSS_gamma_west' entries in the output therefore correspond to
the no-terrain ResUNet (the 'raw' = smoothed-GRAF entries are unchanged).

This will compute BS, reliability, freq use for the test of
Attention ResUNet (NO TERRAIN) with Gamma mixture model.
...
for ithresh, thresh in enumerate(pthresholds):
    ...
    plot_title = 'Relia_GRAF_ResUNet_Mixture_noterrain_MRMS_' + \
...
relia_outfile = os.path.join(relia_dir,
    f'relia_GRAF_ResUNet_Mixture_noterrain_q0.5_{cyyyymmddhh_list[0]}_to_'
