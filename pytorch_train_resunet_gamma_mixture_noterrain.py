"""
pytorch_train_resunet_gamma_mixture_noterrain.py

Usage example:

$ python pytorch_train_resunet_gamma_mixture_noterrain.py 2025120100 12

where you supply the YYYYMMDDHH of initial condition and lead time in h.

*** NO-TERRAIN ABLATION VARIANT ***
This is a fork of pytorch_train_resunet_gamma_mixture.py (v1 architecture,
ResidualBlock + ConvTranspose2d) created to quantify the contribution of the
terrain predictors to forecast skill. It is IDENTICAL to v1 except that the
four terrain-derived input channels are removed:
    - terrain elevation deviation (Delta z)
    - precipitation x terrain interaction
    - zonal terrain gradient (dz/dlon)
    - meridional terrain gradient (dz/dlat)
The model therefore uses only 3 input channels (GRAF precip, GFS column-average
RH, GRAF x RH interaction) instead of 7. Checkpoints use the prefix
"resunet_gamma_mixture_noterrain_" so they are never confused with the
full 7-channel weights.
...
class AttnResUNet(nn.Module):
    ...
    NO-TERRAIN variant: in_channels defaults to 3 (GRAF, GFS RH, GRAF x RH).
    """
    def __init__(self, in_channels=3, num_outputs=6):
...
class GRAF_Dataset(Dataset):
    ...
    def __init__(self, pickle_file, normalization_stats=None, train=False, power_transform=1.0):
        ...
        # NO-TERRAIN feature list: GRAF, RH, GRAF×RH only.
        # The four terrain channels (terrain diff, GRAF×terrain, dlon, dlat) are
        # intentionally dropped for this ablation. terdiff_graf/diff/dlon/dlat are
        # still loaded above but never used here.
        feature_list = [self.graf, self.gfs_r, self.graf_rh_interaction]
        ...
        if normalization_stats is None:
            ...
            maxs[1] = max(maxs[1], 100.0)    # RH (%)
            # GRAF × RH interaction also affected by power transformation
            if self.power_transform == 1.0:
                maxs[2] = max(maxs[2], 7500.0)   # GRAF × RH
            else:
                maxs[2] = max(maxs[2], np.power(75.0, self.power_transform) * 100.0)
            self.stats = {'min': mins, 'max': maxs}
        else:
            self.stats = normalization_stats

        # Normalize all features, pre-stack into (N, 3, H, W), then free individual arrays
        self.graf                = self.normalize(self.graf, 0)
        self.gfs_r               = self.normalize(self.gfs_r, 1)
        self.graf_rh_interaction = self.normalize(self.graf_rh_interaction, 2)
        self.features = np.stack(
            [self.graf, self.gfs_r, self.graf_rh_interaction], axis=1
        ).astype(np.float32)  # (N, 3, 96, 96)
        del self.graf, self.gfs_r, self.graf_rh_interaction
    ...
    def apply_augmentation(self, x, y):
        # NO-TERRAIN variant: the 3 channels (GRAF, RH, GRAF×RH) are all scalar
        # fields, so flips need no channel-sign correction (the dlon/dlat
        # gradient channels that required negation are absent here).
        # Horizontal flip
    ...
    def __getitem__(self, idx):
        x = self.features[idx].copy()  # (3, 96, 96) — copy avoids writing into the shared array
...
def print_diagnostics(epoch, batch_idx, loss_val, logits, targets,
    ...
    with torch.no_grad():
        ...
        def run_synthetic(precip_mm, rh_pct):
            ...
            # NO-TERRAIN: only 3 channels (GRAF, RH, GRAF×RH)
            f0 = p_val              # GRAF precip (transformed)
            f1 = rh_pct             # RH
            f2 = p_val * rh_pct     # GRAF × RH
            ...
            # Create Tensor (3 channels)
            syn_x = torch.zeros((1, 3, 96, 96), device=DEVICE)
...
def train_model(date_str, lead_time_str):
    ...
    print(f"  Input channels: 3 (GRAF, GFS RH, GRAF x RH) -- NO TERRAIN")
    ...
    # Create model with 6 outputs -- NO TERRAIN: 3 input channels
    model = AttnResUNet(in_channels=3, num_outputs=6).to(DEVICE)
    ...
    checkpoint_path = f"{TRAIN_DIR}/resunet_gamma_mixture_noterrain_{date_str}_{lead_time_str}h_best.pth"
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python pytorch_train_resunet_gamma_mixture_noterrain.py YYYYMMDDHH lead_time")
        print("Example: python pytorch_train_resunet_gamma_mixture_noterrain.py 2025120100 12")
        sys.exit(1)
